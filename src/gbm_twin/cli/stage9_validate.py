...
from gbm_twin.workflows.stage9_validation import validate_stage9_model
...
DEFAULT_CACHE_ROOT = Path(
    "results/cache/stage9-reserve-validation-v1"
)
DEFAULT_OUTPUT_DIR = Path(
    "results/cohort/stage9-internal-validation-v1"
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the sealed Stage 9 model on a sealed reserve cohort. "
            "Each patient D/rho is frozen from t0/t1 before t2 is opened."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = validate_stage9_model(
            ...
            stage9_validation_plan_root=_resolve(
                repo_root,
                args.stage9_validation_plan_root,
            ),
        ...
        if not isinstance(summary, dict):
            raise ValueError("Generated Stage 9 validation has no summary")
    except (OSError, ValueError, RuntimeError) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1

    print()
    print(f"Stage 9 validation created: {result.directory}")
