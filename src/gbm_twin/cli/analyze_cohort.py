...
from gbm_twin.workflows.cohort_analysis import analyze_sealed_cohort
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "already sealed cohort evaluation."
        )
    )
    parser.add_argument(
        "--repo-root",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        mean_dice = _summary_float(summary, "mean_twin_dice")
    ...
    print(
        f"Cohort {model_version} scientific analysis created: "
        f"{result.directory}"
    )
    if mean_dice is not None:
        print(f"Twin mean Dice: {mean_dice:.4f}")
