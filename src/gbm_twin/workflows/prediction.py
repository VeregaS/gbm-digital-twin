...
ManifestFactory = Callable[[dict[str, Path]], dict[str, Any]]
...
def _build_manifest(
    *,
    ...
    prediction_duration = target.target_day - observed.days_from_baseline
    ...
    return {
        ...
        "prediction_horizon": {
            ...
            "target_timepoint": target.timepoint_name,
            "target_day": target.target_day,
            "duration_days": prediction_duration,
        },
        "parameters": {
    ...
    *,
    ...
    if destination.exists():
        raise FileExistsError(
            f"Prediction artifact already exists: {destination}"
        )
...
def freeze_v2_prediction(
    *,
    ...
    if (
        ...
        raise ValueError(
            "Frozen V2 protocol requires the t0 -> t1 -> t2 sequence"
        )

    if start.patient_id != observed.patient_id:
        raise ValueError(
            "Prediction timepoints belong to different patients"
        )

    if target.target_day <= observed.days_from_baseline:
        raise ValueError(
            "Prediction target must occur after the observed timepoint"
        )

    if output_dir.resolve().exists():
        raise FileExistsError(
            f"Prediction artifact already exists: {output_dir.resolve()}"
        )
    ...
    voxel_volume_cm3 = (
        float(np.prod(observed.spacing))
        / 1000.0
    )

    start_volume_cm3 = (
        float(np.count_nonzero(start_mask))
        * voxel_volume_cm3
    )
...
def load_frozen_v2_prediction(
    ...
    if not manifest_path.is_file():
        raise FileNotFoundError(
            f"Prediction manifest not found: {manifest_path}"
        )

    if not seal_path.is_file():
        raise FileNotFoundError(
            f"Prediction manifest seal not found: {seal_path}"
        )
    ...
    if _sha256(manifest_path) != expected_manifest_sha256:
        raise ValueError(
            "Prediction manifest checksum mismatch"
        )
    ...
    if manifest.get("schema_version") != V2_PREDICTION_ARTIFACT_SCHEMA_VERSION:
        raise ValueError(
            "Unsupported prediction artifact schema version"
        )

    if manifest.get("protocol_version") != V2_FROZEN_PROTOCOL_VERSION:
        raise ValueError(
            "Unsupported frozen V2 protocol version"
        )

    if (
        manifest.get("model_version") != "V2"
        or manifest.get("sealed") is not True
    ):
        raise ValueError(
            "Prediction artifact is not a sealed V2 artifact"
        )
    ...
    if not required_provenance_fields.issubset(
        provenance
    ):
        raise ValueError(
            "Prediction manifest provenance is incomplete"
        )
    ...
    if (
        ...
        or not isinstance(array_metadata, dict)
    ):
        raise ValueError(
            "Prediction manifest is missing geometry or array metadata"
        )
    ...
    for name, expected_file in _ARRAY_FILES.items():
        ...
        if (
            ...
            raise ValueError(
                f"Invalid metadata for artifact array: {name}"
            )
        ...
        if not path.is_file():
            raise FileNotFoundError(
                f"Prediction artifact array not found: {path}"
            )

        if _sha256(path) != metadata.get("sha256"):
            raise ValueError(
                f"Prediction artifact checksum mismatch: {name}"
            )
        ...
        if (
            ...
            raise ValueError(
                f"Prediction artifact shape mismatch: {name}"
            )

        if str(array.dtype) != metadata.get("dtype"):
            raise ValueError(
                f"Prediction artifact dtype mismatch: {name}"
            )
    ...
    if (
        not np.all(np.isfinite(prediction_field))
        or np.any(
            (prediction_field < 0.0)
            | (prediction_field > 1.0)
        )
    ):
        raise ValueError(
            "Prediction field must contain finite values within [0, 1]"
        )
    ...
    for name in (
        ...
        if not np.all(
            ...
            raise ValueError(
                f"Prediction artifact mask is not binary: {name}"
            )
